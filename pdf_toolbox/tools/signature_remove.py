...
from ..models import SignatureRemovalOptions
from ..services.signature_removal import remove_pdf_signatures


class SignatureRemovalPanel:
    def __init__(self) -> None:
        ...
        self.keep_appearance_var = tk.BooleanVar(value=True)
    ...
    def build(self, parent: ttk.Frame) -> None:
        ...
        option_frame = ttk.LabelFrame(container, text="处理选项", padding=14)
        option_frame.pack(fill="x", pady=(6, 14))

        keep_check = ttk.Checkbutton(
            option_frame,
            text="保留签名图案（默认）",
            variable=self.keep_appearance_var,
        )
        ...
        run_button = ttk.Button(action_row, text="执行签名移除", command=self._run)
    ...
    def _run(self) -> None:
        ...
        options = SignatureRemovalOptions(
            ...
            keep_signature_appearance=self.keep_appearance_var.get(),
        )

        self.status_var.set("正在处理，请稍候...")

        result = remove_pdf_signatures(options)
